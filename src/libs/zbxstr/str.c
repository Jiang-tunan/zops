...
int	zbx_strcasecmp(const char *s1, const char *s2)
{
	...
	int s1len = strlen(s1);
	int s2len = strlen(s2);
	if (s1len != s2len || (0 == s1len && 0 == s2len))
		return s1len - s2len;
...
int zbx_split(char *src, const char *separator, char **dest, int *num)
{
    ...
    if (separator == NULL || strlen(separator) == 0)
        return FAIL;
    pNext = strtok_r(src, separator, &pSave);
    while (pNext != NULL && count < *num)
    {
        *dest ++ = pNext;
        count ++;
        pNext = strtok_r(NULL, separator, &pSave);
    }
    *num = count;
	return SUCCEED;
}
...
int zbx_strrchr(char *str, char split, char **lstr, char **rstr)
{
    ...
    if(index == -1){
		*lstr = zbx_strdup(NULL, str);
		return -1;
	}
	
    int llen = index;
	size_t  alloc = 0, offset = 0; 
	zbx_str_memcpy_alloc(lstr, &alloc, &offset, str, llen); 

	
    int rpos = index + 1;
    int rlen = len - 1 - index;
	alloc = 0, offset = 0; 
	zbx_str_memcpy_alloc(rstr, &alloc, &offset, str + rpos, rlen);
    return index; 
}
...
int get_version(char *version)
{
	...
	int num = 3, iversion=0;
	char *sversion[3] = {0}; 
	int kv[3]={10000,100,1};
	zbx_split(version, ".", sversion, &num);
	if(num >= 3){
		for(int i = 0; i < num; i ++){
			iversion += zbx_atoi(sversion[i]) * kv[i];
		}
	}
	return iversion;
}
