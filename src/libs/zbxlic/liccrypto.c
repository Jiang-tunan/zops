...
int cleanup_on_error(EVP_CIPHER_CTX *ctx, int error_code)
{
    if (ctx)
...
int encrypt(const uchar *src, int srclen, uchar **target, const uchar *session_key)
{
    ...
    if (!crypto_check(ctx, src, srclen, target, session_key))
        return cleanup_on_error(ctx, EENCFAIL);
  

    int tlen, flen;

    if (!EVP_EncryptInit_ex(ctx, EVP_bf_cbc(), NULL, session_key, NULL))
        return cleanup_on_error(ctx, EENCINIT);
    ...
    if (!EVP_EncryptUpdate(ctx, *target, &tlen, src, srclen))
        return cleanup_on_error(ctx, EENCUPDT);

    if (!EVP_EncryptFinal_ex(ctx, *target + tlen, &flen))
        return cleanup_on_error(ctx, EENCFINL);
...
int decrypt(const uchar *src, int srclen, uchar **target, const uchar *session_key)
{
    ...
    if (!crypto_check(ctx, src, srclen, target, session_key))
        return cleanup_on_error(ctx, EDECFAIL);

    int tlen, flen;

    if (!EVP_DecryptInit_ex(ctx, EVP_bf_cbc(), NULL, session_key, NULL))
        return cleanup_on_error(ctx, EDECINIT);
    ...
    if (!EVP_DecryptUpdate(ctx, *target, &tlen, src, srclen))
        return cleanup_on_error(ctx, EDECUPDT);

    if (!EVP_DecryptFinal_ex(ctx, *target + tlen, &flen))
        return cleanup_on_error(ctx, EDECFINL);
