...
void	zbx_ipaddress_to_ip(char *ip, const int *ipaddress, const unsigned char type)
{
#ifdef HAVE_IPV6
			if (ZBX_IPRANGE_V6 == type)
			{
				zbx_snprintf(ip, ZBX_INTERFACE_IP_LEN_MAX, "%x:%x:%x:%x:%x:%x:%x:%x", (unsigned int)ipaddress[0],
			...
			{
#endif
				zbx_snprintf(ip, ZBX_INTERFACE_IP_LEN_MAX, "%u.%u.%u.%u", (unsigned int)ipaddress[0],
						(unsigned int)ipaddress[1], (unsigned int)ipaddress[2],
