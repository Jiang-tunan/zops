...
func main() {
	...
	const (
		...
		foregroundDescription = "Run Tognix agent in foreground"
	)
	...
	greeting := fmt.Sprintf("Starting Tognix Agent 2 (%s)", version.Long())
	log.Infof(greeting)
	...
	agent.FirstHostname = hostnames[0]
	hostmessage := fmt.Sprintf("Tognix Agent2 hostname: [%s]", agent.Options.Hostname)
	...
	farewell := fmt.Sprintf("Tognix Agent 2 stopped. (%s)", version.Long())
	log.Infof(farewell)
...
func fatalExit(message string, err error) {
	...
	fmt.Fprintf(os.Stderr, "tognix_agent2 [%d]: ERROR: %s\n", os.Getpid(), message)
	os.Exit(1)
}
