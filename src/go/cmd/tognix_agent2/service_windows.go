...
func loadOSDependentFlags() {
	const (
		...
		svcInstallDescription = "Install Tognix agent 2 as service"
	)
	...
	const (
		...
		svcUninstallDescription = "Uninstall Tognix agent 2 from service"
	)
	...
	const (
		...
		svcStartDescription = "Start Tognix agent 2 service"
	)
	...
	const (
		...
		svcStopDescription = "Stop Tognix agent 2 service"
	)
...
func resolveWindowsService(confPath string) error {
	...
	msg = fmt.Sprintf("tognix_agent2 [%d]: %s\n", os.Getpid(), msg)
...
func svcInstall(conf string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get Tognix Agent 2 exeutable path: %s", err.Error())
	}
...
func runService() {
	if err := svc.Run(serviceName, &winService{}); err != nil {
		fatalExit("use foreground option to run Tognix agent as console application", err)
		return
	}
}
...
func (ws *winService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (ssec bool, errno uint32) {
	...
	case <-fatalStopChan:
		...
		// This is needed to make sure that windows will receive the status stopped before tognix agent 2 process ends
