		...
		size_t *value_alloc)
{
	...
	" snmpv3_contextname:%s\n type:%d\n snmpv3_securitylevel:%hhu\n snmpv3_authprotocol:%hhu\n snmpv3_privprotocol:%hhu\n houseid:%d\n druleid:%d\n"
	" ip:%s\n port:%d timeout:%d",
	__func__,dcheck->dcheckid,dcheck->ports,dcheck->key_,dcheck->snmp_community,dcheck->snmpv3_securityname,dcheck->snmpv3_authpassphrase,dcheck->snmpv3_privpassphrase,
	dcheck->snmpv3_contextname,dcheck->type,dcheck->snmpv3_securitylevel,dcheck->snmpv3_authprotocol,dcheck->snmpv3_privprotocol,dcheck->houseid,dcheck->druleid,
	ip,port,config_timeout);
	...
	if (SUCCEED == ret)
	{
		...
		switch (dcheck->type)
		{
			...
			case SVC_IPMI:
				...
				get_ipmi_value_by_ip(ip, port);
				zabbix_log(LOG_LEVEL_INFORMATION, "#ZOPS#IPMI %s() dc.type=%d IP=%s ret=%d",
													 __func__, dcheck->type, ip, ret);
				break;
			default:
				break;
		}
		...
		zbx_vector_ptr_t *services, zbx_vector_ptr_t *dchecks, zbx_vector_uint64_t *dcheckids, int config_timeout)
{
	...
	offset += zbx_snprintf(sql + offset, sizeof(sql) - offset,
			"select dc.dcheckid,dc.type,dc.key_,dc.snmp_community,dc.snmpv3_securityname,dc.snmpv3_securitylevel,"
				"dc.snmpv3_authpassphrase,dc.snmpv3_privpassphrase,dc.snmpv3_authprotocol,dc.snmpv3_privprotocol,"
				"dc.ports,dc.snmpv3_contextname,dr.houseid,dc.druleid"
			" from dchecks dc join drules dr on dc.druleid=dr.druleid"
	...
	while (NULL != (row = zbx_db_fetch(result)))
	{
		...
		dcheck->snmp_community = zbx_strdup(NULL, row[3]);
		dcheck->snmpv3_securityname = zbx_strdup(NULL, row[4]);
		dcheck->snmpv3_securitylevel = (unsigned char)atoi(row[5]);
		dcheck->snmpv3_authpassphrase = zbx_strdup(NULL, row[6]);
		dcheck->snmpv3_privpassphrase = zbx_strdup(NULL, row[7]);
		dcheck->snmpv3_authprotocol = (unsigned char)atoi(row[8]);
		dcheck->snmpv3_privprotocol = (unsigned char)atoi(row[9]);
		dcheck->ports = zbx_strdup(NULL, row[10]);
		dcheck->snmpv3_contextname = zbx_strdup(NULL, row[11]);
		dcheck->houseid = atoi(row[12]);
	...
	long ctime = time(NULL) - btime;
	zabbix_log(LOG_LEVEL_WARNING, "#ZOPS#discoverer. druleid=%d,cost_time=%lld",drule->druleid, ctime);
}
		...
		int now, const zbx_vector_ptr_t *services, const zbx_vector_ptr_t *dchecks, zbx_vector_uint64_t *dcheckids)
{
	...
	for (i = 0; i < services->values_num; i++)
	{
		...
		if (FAIL == (index = zbx_vector_ptr_search(dchecks,&service->dcheckid, ZBX_DEFAULT_UINT64_PTR_COMPARE_FUNC)))
		{
...
static void	process_rule(zbx_db_drule *drule, int config_timeout)
{
	...
	for (start = drule->iprange; '\0' != *start;)
	{
		...
		do
		{
			...
			process_checks(drule, &host_status, ip, 0, now, &services, &dchecks, &dcheckids, config_timeout);
...
static int	process_discovery(time_t *nextcheck, int config_timeout)
{
	...
	else if (SUCCEED == zbx_dc_drule_next(now, &druleid, nextcheck))
		;
	else
		return 0;
	...
	do
	{
		...
		if (NULL != (row = zbx_db_fetch(result)))
		{
			...
			else
			{
				...
				//解析规则 一个规则用完以后会去数据库load下一个规则
				process_rule(&drule, config_timeout);
			}

			//只有自动扫描的才重复加入循环扫描队列里面
			if (!is_user_discover)
		...
		//先看一下是不是跑的用户扫描 如果没有就走自动扫描
		if (SUCCEED == user_discover_next_druleid(&druleid))
			is_user_discover = 1;
		else
			is_user_discover = 0;

		if (!is_user_discover && SUCCEED != zbx_dc_drule_next(now, &druleid, nextcheck))
			break;
	}
...
// discover进程等待通知机制相关变量
pthread_cond_t g_disc_cond;
pthread_mutex_t g_disc_mutex;

// 通知discover进程不要waiting,立刻处理
 ...
 *   超时：110, 通知退出:0
*/
int wait_discover_thread(int sec)
{
    int ret = 0;
    struct timeval now;
    struct timespec tm;
...
#define STAT_INTERVAL	5	/* if a process is busy and does not sleep then update status not faster than */
	...
	// 初始化notify/waiting机制相关参数
	pthread_cond_init(&g_disc_cond, NULL);
	...
	discovery_rules_discoverer_thread_init(); // 接收线程初始化

	// discoverer_manager_t *dm = (discoverer_manager_t *)zbx_malloc(NULL, sizeof(discoverer_manager_t));
	// if (SUCCEED !=discoverer_manager_init(dm, error))
	// {
	// 	zbx_free(error);
	// 	return FAIL;
	// }

	// int workers_num = DISCOVERER_WORKER_NUM;
	// zbx_discoverer_worker_t *workers = (zbx_discoverer_worker_t *)zbx_calloc(NULL, (size_t)workers_num, sizeof(zbx_discoverer_worker_t));
	// if (SUCCEED !=discoverer_workers_init(workers, workers_num))
	// {
	// 	zbx_free(error);
	// 	return FAIL;
	// }
	...
	while (ZBX_IS_RUNNING() && LIC_IS_SUCCESS())
	{
		...
		// discover进程等待，一直到超时或被通知
		int ret_wait = wait_discover_thread(sleeptime); 
		if (0 != ret_wait && SUCCEED == zbx_rtc_wait(&rtc, info, &rtc_cmd, &rtc_data, 0) && 0 != rtc_cmd)
		{
