...
static zbx_config_log_t	log_file_cfg = {NULL, NULL, LOG_TYPE_UNDEFINED, 100}; // 修改日志默认大小为100M，解决1M会导致莫名其妙问题
...
static int	server_startup(zbx_socket_t *listen_sock, int *ha_stat, int *ha_failover, zbx_rtc_t *rtc)
{
	...
	for (i = 0; i < threads_num; i++)
	{
		...
		// 软件许可校验不通过，则只启动trapper进程，trapper进程用来获得product key
		int process_type = (int)thread_args.info.process_type;
		if(!LIC_IS_SUCCESS() && process_type != ZBX_PROCESS_TYPE_TRAPPER)
		 	continue;
