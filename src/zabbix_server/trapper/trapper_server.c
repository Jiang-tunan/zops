...
#include "../discoverer/discoverer_protocol.h"
...
// //负责把web端的参数发到discoverer & 简单的校验 op放在json数据里面
// void	trapper_process_discoverer_send(zbx_socket_t *sock, const struct zbx_json_parse *jp, const char *json_in)
// {
...
// 	char	*json_discoverer = NULL, *error=NULL;

// 	zbx_user_t		user;
// 	zbx_user_init(&user);
// 	struct zbx_json	json_out;
// 	zbx_json_init(&json_out, ZBX_JSON_STAT_BUF_LEN);
...
// 	// zbx_uint32_t size = zbx_discoverer_serialize_json(&request, json_in);

// 	//trapper<->discover
// 	if (SUCCEED != zbx_ipc_async_exchange(ZBX_IPC_SERVICE_DISCOVERER, ZBX_IPC_TRAPPER_SEND_SINGLE_SCAN, SEC_PER_MIN, request, size, &response, &error))
// 	{
// 		goto out;
// 	}

// 	zbx_discoverer_deserialize_json(response, &json_discoverer);

// out:

// 	if (json_discoverer)
// 	{
// 		zabbix_log(LOG_LEVEL_INFORMATION,"discoverer->web buf:%s", json_discoverer);
// 		(void)zbx_tcp_send(sock, json_discoverer);
// 	}
// 	else
// 	{
// 		zbx_json_addint64(&json_out, "ret", ret);
// 		zbx_json_addstring(&json_out, "request", json_in, ZBX_JSON_TYPE_STRING);
// 		zbx_json_addstring(&json_out, "response", error, ZBX_JSON_TYPE_STRING);
// 		zabbix_log(LOG_LEVEL_INFORMATION,"trapper->web buf:%s", json_out.buffer);
// 		(void)zbx_tcp_send(sock, json_out.buffer);
// 	}

// 	zbx_free(error);
// 	zbx_free(request);
// 	zbx_free(json_discoverer);
// 	zbx_free(response);
// 	zbx_json_free(&json_out);
// 	zbx_user_free(&user);
// }
...
int	trapper_process_request(const char *request, zbx_socket_t *sock, const struct zbx_json_parse *jp,
		...
		zbx_get_program_type_f get_program_type_cb, int config_timeout, const char *sockbuf)
{
	...
	// else if (0 == strcmp(request, ZBX_PROTO_VALUE_ZABBIX_DISCOVERER_SEND))
	// {
	// 	trapper_process_discoverer_send(sock, jp, sockbuf);
	// }
