...
/******************************************************************************
 *                                                                            *
 * Purpose: check host house condition                                        *
 *                                                                            *
 * Parameters: esc_events - [IN] events to check                              *
 *             condition  - [IN/OUT] condition for matching, outputs          *
 *                                   event ids that match condition           *
 ...
 *                                                                            *
 ******************************************************************************/
static int	check_host_house_idc_condition(const zbx_vector_ptr_t *esc_events, zbx_condition_t *condition)
{
	char			*sql = NULL;
	const char		*operation,*field;
	size_t			sql_alloc = 0, sql_offset = 0;
	DB_RESULT		result;
	DB_ROW			row;
	zbx_vector_uint64_t	objectids;
	...
	if (ZBX_CONDITION_TYPE_HOST_HOUSE == condition->conditiontype)
		field = "houseid";
	else if (ZBX_CONDITION_TYPE_HOST_IDC == condition->conditiontype)
		field = "idcid";
	else
		return NOTSUPPORTED;
	...
	zbx_snprintf_alloc(&sql, &sql_alloc, &sql_offset,
			...
			" where h.hostid=i.hostid"
				...
				"%s h.%s=" ZBX_FS_UI64
				" and",
			operation,field,condition_value);
...
static void	check_trigger_condition(const zbx_vector_ptr_t *esc_events, zbx_condition_t *condition)
{
	...
	switch (condition->conditiontype)
	{
		...
		case ZBX_CONDITION_TYPE_HOST:
			...
			break;
		case ZBX_CONDITION_TYPE_HOST_HOUSE:
		case ZBX_CONDITION_TYPE_HOST_IDC:
			ret = check_host_house_idc_condition(esc_events, condition);
			break;
 ...
 *                                                                            *
 ******************************************************************************/
static int	check_intern_host_house_idc_condition(const zbx_vector_ptr_t *esc_events, zbx_condition_t *condition)
{
	char			*sql = NULL;
	const char		*operation, *operation_item, *field;
	size_t			sql_alloc = 0, i;
	DB_RESULT		result;
	DB_ROW			row;
	int			objects[3] = {EVENT_OBJECT_TRIGGER, EVENT_OBJECT_ITEM, EVENT_OBJECT_LLDRULE};
	...
	for (i = 0; i < (int)ARRSIZE(objects); i++)
	{
		...
		else	/* EVENT_OBJECT_ITEM, EVENT_OBJECT_LLDRULE */
		{
			zbx_snprintf_alloc(&sql, &sql_alloc, &sql_offset,
					...
					"%s h.%s=" ZBX_FS_UI64
						" and",
					operation_item,field,condition_value);
...
static void	check_internal_condition(const zbx_vector_ptr_t *esc_events, zbx_condition_t *condition)
{
	...
	switch (condition->conditiontype)
	{
		...
		case ZBX_CONDITION_TYPE_HOST_HOUSE:
		case ZBX_CONDITION_TYPE_HOST_IDC:
			ret = check_intern_host_house_idc_condition(esc_events, condition);
			break;
		case ZBX_CONDITION_TYPE_EVENT_TAG:
			check_condition_event_tag(esc_events, condition);
			ret = SUCCEED;
