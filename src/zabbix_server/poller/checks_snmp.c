		...
		zbx_snmp_walk_cb_func walk_cb_func, void *walk_cb_arg)
{
	...
	while (1 == running)
	{
		...
		zabbix_log(LOG_LEVEL_DEBUG, "%s() snmp_synch_response() status:%d s_snmp_errno:%d errstat:%ld"
				" max_vars:%d,flags:%d", __func__, status, ss->s_snmp_errno,
				NULL == response ? (long)-1 : response->errstat, max_vars,item->flags);

		// 如果是扫码发现规则，则不需要重试机制，加快扫描速度,要放在第一个判断
		if ( (STAT_SUCCESS != status || SNMP_ERR_NOERROR != response->errstat)
		      && (ZBX_FLAG_DISCOVERY_RULE & item->flags))
		{
...
reduce_max_vars:
		...
		for (num_vars = 0, var = response->variables; NULL != var; num_vars++, var = var->next_variable)
		{
			...
			//discovery[ 这种格式的oid key只能查父节点 不能查到子节点
